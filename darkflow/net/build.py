...
class TFNet(object):
	...
	def setup_meta_ops(self):
		...
		if utility > 0.0:
			self.pyttsx3('GPU mode with {} usage'.format(utility))
			cfg['gpu_options'] = tf.GPUOptions(
				per_process_gpu_memory_fraction = utility)
			cfg['allow_soft_placement'] = True
		else: 
			self.pyttsx3('Running entirely on CPU')
	...
	def savepb(self):
		...
		self.pyttsx3('Saving const graph def to {}'.format(name))
